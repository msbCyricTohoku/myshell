/*======================================================
  ...
  Features:
   ...
   - Quote-Safe Tokenizer ("text | inside")
   - I/O Redirection (>, >>, <)
   - Env Vars ($VAR, $?, ~) Expansion
   - Grid-Style Tab Autocompletion Menu
   - Free Cursor Navigation (Left/Right, Del)
======================================================*/
...
/* Interactive Key Loop (Arrows, Tabs, Insert) */
int read_input(char *line) {
  ...
  while (reading) {
    ...
    snprintf(prompt, sizeof(prompt), "\r\033[2K%s%s@%s%s:%s%s %s❯%s ",
             COLOR_CYAN, user, host, COLOR_RESET, DIR_COLOR, d_cwd,
    ...
    case 27: { /* Arrows & Edit Keys */
      ...
      if (read(STDIN_FILENO, &seq[0], 1) == 1 && seq[0] == '[' &&
        ...
        if (seq[1] == 'A') {
          ...
          if (history_pos < history_count) {
            strcpy(line, ++history_pos == history_count ? cur_in
                                                        : history[history_pos]);
            pos = len = strlen(line);
          }
        } else if (seq[1] == 'C') {
    ...
    case 9: {
      if (!tab_mode) {
        ...
        while (word_pos > 0 && line[word_pos - 1] != ' ' &&
          ...
          word_pos--;
        char w[MAX_LINE];
        strncpy(w, &line[word_pos], pos - word_pos);
        w[pos - word_pos] = '\0';
        ...
        if (is_first && !strchr(w, '/') && strlen(w) > 0) {
          const char *blt[] = {"cd", "exit", "clear", "history", "ll", NULL};
          for (int i = 0; blt[i]; i++)
          ...
          if (slash) {
            strncpy(d_path, w, slash - w + 1);
            d_path[slash - w + 1] = '\0';
            strcpy(pref, slash + 1);
            if (slash > w) {
              strncpy(s_dir, w, slash - w);
              s_dir[slash - w] = '\0';
            } else
              strcpy(s_dir, "/");
          } else
...
/* 1. Pre-pads operators so tokenizer easily isolates them natively without
 * needing spaces */
...
/* 2. Isolates strings natively handling spaces perfectly inside quotes */
...
/* 3. Expands Env vars ($), Tildes (~), and Globs (*, ?) safely */
void expand_args(char **args, char **new_args, char buf[][MAX_LINE]) {
  ...
  for (int i = 0; args[i] && n < MAX_ARGS - 1; i++) {
    ...
    /* Safely expand variables & strip quotes */
    ...
    *w = '\0';
    ...
    if (temp[0] == '~' && (!temp[1] || temp[1] == '/')) {
      char *h = getenv("HOME");
      char t2[MAX_LINE];
      snprintf(t2, MAX_LINE, "%s%s", h ? h : "", temp + 1);
      strcpy(temp, t2);
    }
    ...
    if (!has_quote && strpbrk(temp, "*?")) {
      ...
      if (glob(temp, GLOB_NOCHECK, NULL, &g) == 0) {
        for (size_t j = 0; j < g.gl_pathc && n < MAX_ARGS - 1; j++) {
          strncpy(buf[n], g.gl_pathv[j], MAX_LINE);
          new_args[n++] = buf[n];
        }
      }
      globfree(&g);
    } else {
      strncpy(buf[n], temp, MAX_LINE);
      new_args[n++] = buf[n];
    }
  }
  new_args[n] = NULL;
...
/* 4. Route file paths over process handles cleanly via Kernel FDs */
...
/* 5. Primary Execution Core (Handles pipes arrays natively) */
int execute_pipeline(char **args, int bg) {
  ...
  /* Built-ins (No forks for parent-modifying commands) */
  if (num_cmds == 1) {
  ...
  for (int c = 0; c < num_cmds; c++) {
    ...
    *cmd_end = NULL;
    ...
    /* Alias Auto-Injection Engine */
    if (pipe_args[0]) {
      if (!strcmp(pipe_args[0], "ll")) {
        pipe_args[0] = "ls";
        for (int x = p_idx; x >= 1; x--)
          pipe_args[x + 2] = pipe_args[x];
        pipe_args[1] = "-al";
        pipe_args[2] = "--color=auto";
      } else if (!strcmp(pipe_args[0], "ls") || !strcmp(pipe_args[0], "grep")) {
        for (int x = p_idx; x >= 1; x--)
          pipe_args[x + 1] = pipe_args[x];
        pipe_args[1] = "--color=auto";
      }
    }
    ...
    pids[c] = fork();
    if (pids[c] == 0) {
      signal(SIGINT, SIG_DFL); /* Restore native behavior */
      ...
      handle_redirections(
          pipe_args); /* Fire redirects AFTER pipes so > overrides stdout */

      if (pipe_args[0]) {
        if (!strcmp(pipe_args[0], "cd") || !strcmp(pipe_args[0], "exit") ||
            !strcmp(pipe_args[0], "clear"))
          exit(0);
        if (!strcmp(pipe_args[0], "history")) {
          for (int h = 0; h < history_count; h++)
            printf("%d  %s\n", h + 1, history[h]);
          exit(0);
        }
  ...
  if (!bg) {
    ...
    printf("[%d] background task running\n", pids[num_cmds - 1]);
    return 0;
  }
}

/* 6. Recursive Logic Evaluator (&&, ||, ;, &) natively executes left-to-right
 */
int execute_logic(char **args) {
  ...
  while (args[i]) {
    ...
    int is_sem = !strcmp(args[i], ";"), is_bg = !strcmp(args[i], "&");

    if (is_and || is_or || is_sem || is_bg) {
      chunk[c] = NULL;
      if (c > 0 && !skip)
        last_status = execute_pipeline(chunk, is_bg);
  ...
  chunk[c] = NULL;
  if (c > 0 && !skip)
    last_status = execute_pipeline(chunk, 0);
  return last_status;
}

int main() {
  tcgetattr(STDIN_FILENO, &orig_termios);
  atexit(disable_raw_mode); /* Safely restore terminal state when exit is
                               triggered */

  char line[MAX_LINE], padded[MAX_LINE * 2];
  char *raw_args[MAX_ARGS], *args[MAX_ARGS];
  static char exp_buf[MAX_ARGS][MAX_LINE];

  signal(SIGINT, SIG_IGN); /* Protect parent from rogue process terminations */

  while (1) {
    while (waitpid(-1, NULL, WNOHANG) > 0)
      ; /* Reap Zombies in background quietly */
    if (!read_input(line))
      break;
    add_history(line);

    pad_operators(line, padded);
