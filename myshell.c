    ...
    case 9: {
      if (!tab_mode) {
        ...
        if (is_first && !strchr(w, '/') && strlen(w) > 0) {
          const char *blt[] = {"cd",      "exit",    "clear",
                               "history", "export",  "source",
                               "alias",   "unalias", NULL};
          for (int i = 0; blt[i]; i++)
            if (!strncmp(blt[i], w, strlen(w))) {
              snprintf(matches[match_count].insert, MAX_LINE, "%s", blt[i]);
              snprintf(matches[match_count].display, MAX_LINE, "%s", blt[i]);
          ...
          for (int i = 0; i < alias_count; i++)
            if (!strncmp(aliases[i].name, w, strlen(w))) {
              ...
              for (int k = 0; k < match_count; k++)
                if (!strcmp(matches[k].display, aliases[i].name))
                  dup = 1;
              if (!dup) {
          ...
          if (p_env) {
            snprintf(p_copy, sizeof(p_copy), "%s", p_env);
        ...
        if (match_count > 0) {
          ...
          if (match_count == 1) {
            ...
            char lcp[MAX_LINE];
            snprintf(lcp, MAX_LINE, "%s", matches[0].insert);
    ...
    while (*r) {
      if (*r == '\'' && !dq) {
        ...
        if (*r == '?') {
          ...
          char var[64];
          int v = 0;
          while (isalnum((unsigned char)*r) || *r == '_')
            var[v++] = *r++;
          var[v] = '\0';
          char *val = getenv(var);
          if (val) {
...
int execute_pipeline(char **args, int bg) {
  ...
  for (int c = 0; c < num_cmds; c++) {
    ...
    *cmd_end = NULL;
    ...
    if (num_cmds == 1) {
      if (!strcmp(pipe_args[0], "exit"))
        exit(last_status);
      if (!strcmp(pipe_args[0], "clear")) {
        printf("\033[H\033[2J");
        return 0;
      }
      ...
      if (!strcmp(pipe_args[0], "cd")) {
        char *t = pipe_args[1] ? pipe_args[1] : getenv("HOME");
        if (pipe_args[1] && !strcmp(pipe_args[1], "-"))
      ...
      if (!strcmp(pipe_args[0], "export")) {
        for (int i = 1; pipe_args[i]; i++) {
          char *eq = strchr(pipe_args[i], '=');
          if (eq) {
            *eq = '\0';
            setenv(pipe_args[i], eq + 1, 1);
          }
        }
        return 0;
      }
      if (!strcmp(pipe_args[0], "source")) {
        if (pipe_args[1])
          load_rc(pipe_args[1]);
        else
      ...
      if (!strcmp(pipe_args[0], "alias")) {
        if (!pipe_args[1]) {
          ...
          for (int i = 1; pipe_args[i]; i++) {
            ...
            if (eq) {
              *eq = '\0';
              set_alias(pipe_args[i], eq + 1);
            }
          }
        }
        return 0;
      }
      if (!strcmp(pipe_args[0], "unalias")) {
        for (int i = 1; pipe_args[i]; i++)
          remove_alias(pipe_args[i]);
        return 0;
      }
    }
    ...
    if (pids[c] == 0) {
      ...
      if (pipe_args[0]) {
        if (!strcmp(pipe_args[0], "cd") || !strcmp(pipe_args[0], "exit") ||
            !strcmp(pipe_args[0], "clear") || !strcmp(pipe_args[0], "export") ||
            !strcmp(pipe_args[0], "source") || !strcmp(pipe_args[0], "unalias"))
          exit(0);
        if (!strcmp(pipe_args[0], "history")) {
          for (int h = 0; h < history_count; h++)
            printf("%d  %s\n", h + 1, history[h]);
...
void process_command(char *line) {
  char padded[MAX_LINE * 2], *raw_args[MAX_ARGS], *args[MAX_ARGS];
